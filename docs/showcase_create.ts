...
function write(prog: BenchmarkRunProgress) {
  if (
    ...
    (prog.state === ProgressState.BenchStart &&
      prog.running?.name === "for100ForIncrementX1e6") ||
    (prog.state === ProgressState.BenchPartialResult &&
      prog.running?.name === "for100ForIncrementX1e8_long" &&
      prog.running.measuredRunsMs.length == 52)
  ) {
    progress.push(prog);
    writeJsonSync(
